...
void CSPSolver::SetVariables(const vector<Group>& groups, const vector<Day>& days, int LESSONS_PER_DAY) {
    ...
    for (const Day& day: days) {
        for (size_t lesson = 1; lesson <= LESSONS_PER_DAY; ++lesson) {
            // for all lessons in each day
            const int start = num;
            for (const Group& group: groups) {
                // for each group
                variables_storage.emplace_back(num, Time{day, lesson}, group); // deque
                free_variables.insert(&variables_storage.back()); // set
                ++num;
            }
            const int finish = num - 1; // last assigned id
...
void CSPSolver::SetDomains(const vector<Room>& rooms) {
    for (const Variable& variable: variables_storage) {
        // for each slot in the schedule...
        for (const Subject& subject: variable.group.subjects) {
            // for each subject... ( int the group)
            for (const Teacher& teacher: subject.teachers) {
                // for each teacher that is eligible to teach this course...
...
const Variable* CSPSolver::MRV_Heuristic(map<const Variable *, Domain *>& cur) {
    ...
    for (const Variable* variable: free_variables) {
        const int cnt = domains[variable].size();
        if (cnt < mn) {
            res = variable;
            mn = cnt;
        }
    ...
    for (const Variable* variable: free_variables) {
        ...
        for (const Variable* variable1: variable_neighbours[variable]) {
            ...
            if (!cur.count(variable1)) {
                // current does not have this variable assigned
                ++cnt;
            }
...
bool CSPSolver::check_constraints(map<const Variable *, Domain *>& cur, const Variable* variable1, Domain* domain1) {
    for (const Variable* variable2: variable_neighbours[variable1]) {
        if (cur.count(variable2)) {
            const Domain* domain2 = cur[variable2];
            if (domain1->teacher == domain2->teacher) {
                return false;
            }
            if (domain1->room == domain2->room) {
...
bool CSPSolver::Backtracking(map<const Variable *, Domain *>& cur, map<const Variable *, Domain *>& ans) {
    ...
    for (Domain* domain: OrderDomainValues(domains[variable])) {
        if (check_constraints(cur, variable, domain)) {
            cur[variable] = domain;
            ForwardChecking(variable, domain, cur);
            const bool result = Backtracking(cur, ans);
            if (result) {
                return true;
            }
            cur.erase(variable);
...
CSPSolver::RemoveInconsistentDomains(const Variable* variable, Domain* domain, map<const Variable *, Domain *>& cur) {
    for (const Variable* variable2: variable_neighbours[variable]) {
        if (!cur.count(variable2)) {
            for (int i = 0; i < domains[variable2].size();) {
                const Domain* domain2 = domains[variable2][i];
